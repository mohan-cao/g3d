...
public class DigraphSort3 {
	public static HashMap<Node,TreeSet<Node>> predEdges; //predecessor edges
	public static TreeSet<Node> nodes; //set of nodes 
	public static Set<Node> nodesToRemove;
	public static Set<Node> nodesToAdd;
	...
	private static Node mergeNodes(NodeSeenException ne){
		if(ne._nodes.size()>1){
			...
			for(Node x1 : ne._nodes){
				for(Node x2 : predEdges.keySet()){
					if(x2.equals(x1)){
						x2._client = n;
						x1._client = n;
						System.err.println("Client for node" +x2+"$"+x2.hashCode() + " set to "+x2._client);
					}
				}
	...
	public static int G3Dsort(Node x) throws NodeSeenException{
		if(x._client!=null){System.err.println(x+"$"+x.hashCode() +" has a client");x = x._client;}else{System.err.println(x+"$"+x.hashCode()+" has no client");}
